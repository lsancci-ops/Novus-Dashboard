    ...
    if total:
        ...
        for e in ESTADOS:
            ...
            fig_est.add_trace(go.Bar(
                x=[v / total * 100], y=["e"], orientation="h", name=f"{e} ({v})",
                marker_color=colores_est.get(e, GREEN),
    ...
    if MODO_EDICION:
        ...
        if guardar:
            if not (iniciales or "").strip():
                st.warning("Poné tus iniciales antes de guardar — quedan en el historial del cambio.")
            else:
                try:
                    buf = io.BytesIO()
