...
# ═══════════════════════════════════════════════════════════════
# MÓDULO 1: DASHBOARD DE CONTRAPARTES Y FLUJO
# ═══════════════════════════════════════════════════════════════
if modulo == "📊 Dashboard de Contrapartes":
    ...
    def fmt_usd(v):
        ...
        if a >= 1e9: return f"{s}USD {a/1e9:,.2f}B"
        if a >= 1e6: return f"{s}USD {a/1e6:,.1f}M"
        if a >= 1e3: return f"{s}USD {a/1e3:,.1f}K"
        return f"{s}USD {a:,.0f}"

    def fmt_pct_html(pct, nd_reason=None):
        if nd_reason: return f'<span class="nd" title="{nd_reason}">n/d</span>'
        if pct is None or (isinstance(pct, float) and np.isnan(pct)): return '<span class="nd">—</span>'
    ...
    with st.expander("🔍  Filtros  ·  vacío = todos", expanded=False):
        ...
        with c1: anos_sel = st.multiselect("Año", sorted(df_raw["Año"].unique(), reverse=True), placeholder="Todos")
        with c2:
            meses_disp = [m for m in MESES_ORD if m in set(df_raw["Mes_Nombre"])]
            meses_sel = st.multiselect("Mes", meses_disp, placeholder="Todos")
        with c3: assets_sel = st.multiselect("Asset Category", sorted(df_raw["Asset_Category"].unique()), placeholder="Todas")
        with c4: fondos_sel = st.multiselect("Fondo", sorted(df_raw["Fondo"].unique()), placeholder="Todos")
        with c5: agentes_sel = st.multiselect("Agente", sorted(df_raw["Agente"].unique()), placeholder="Todos")

    fecha_desde, fecha_hasta = HIST_MIN, HIST_MAX
    if preset == "YTD": fecha_desde = pd.Timestamp(year=HIST_MAX.year, month=1, day=1)
    elif preset == "Últimos 30d": fecha_desde = HIST_MAX - pd.Timedelta(days=30)
    elif preset == "Últimos 90d": fecha_desde = HIST_MAX - pd.Timedelta(days=90)
    elif preset == "Últimos 12m": fecha_desde = HIST_MAX - pd.DateOffset(months=12)
    elif preset == "Personalizado" and rango_custom:
        if isinstance(rango_custom, (tuple, list)):
            if len(rango_custom) == 2: fecha_desde, fecha_hasta = pd.Timestamp(rango_custom[0]), pd.Timestamp(rango_custom[1])
            elif len(rango_custom) == 1: fecha_desde = pd.Timestamp(rango_custom[0])
        else: fecha_desde = pd.Timestamp(rango_custom)
    ...
    def aplicar(base, fechas=True, cats=True, agente=True):
        ...
        if fechas:
            ...
            if anos_sel: d = d[d["Año"].isin(anos_sel)]
            if meses_sel: d = d[d["Mes_Nombre"].isin(meses_sel)]
        if cats:
            if assets_sel: d = d[d["Asset_Category"].isin(assets_sel)]
            if fondos_sel: d = d[d["Fondo"].isin(fondos_sel)]
        if agente and agentes_sel: d = d[d["Agente"].isin(agentes_sel)]
        return d
    ...
    else: asset_dom, asset_dom_pct = "—", 0.0
    ...
    else: agente_lider, agente_lider_pct, hhi, top3_pct, top5_pct = "—", 0.0, 0.0, 0.0, 0.0
    ...
    else: mom, mom_label = None, ""

    st.markdown('<div class="section-title">Métricas Clave</div><div class="section-underline"></div>', unsafe_allow_html=True)

    k1, k2, k3, k4 = st.columns(4)
    with k1:
        d_html = (f'<div class="kpi-delta" style="color:{"#2D6A4F" if mom >= 0 else "#E05555"}">{"▲" if mom >= 0 else "▼"} {abs(mom):.1f}% <span style="color:#666;font-weight:400">vs mes anterior ({mom_label})</span></div>') if mom is not None else ""
        st.markdown(f'<div class="kpi-card accent"><div class="kpi-label">Volumen del período</div><div class="kpi-value">{fmt_usd(vol_total)}</div>{d_html}<div class="kpi-sub">{n_ops:,} operaciones</div></div>', unsafe_allow_html=True)
    with k2:
        d_html = (f'<div class="kpi-delta" style="color:{"#2D6A4F" if ytd_delta >= 0 else "#E05555"}">{"▲" if ytd_delta >= 0 else "▼"} {abs(ytd_delta):.1f}% <span style="color:#666;font-weight:400">vs {ytd_prev_start.year}</span></div>') if ytd_delta is not None else '<div class="kpi-delta nd">sin comparable del año anterior</div>'
        st.markdown(f'<div class="kpi-card"><div class="kpi-label">Volumen YTD {HIST_MAX.year}</div><div class="kpi-value">{fmt_usd(vol_ytd)}</div>{d_html}</div>', unsafe_allow_html=True)
    with k3:
        st.markdown(f'<div class="kpi-card"><div class="kpi-label">Agente líder</div><div class="kpi-name" title="{agente_lider}">{agente_lider}</div><div class="kpi-value sm">{agente_lider_pct:.1f}%</div><div class="kpi-sub">sobre {len(ag_vol)} agentes activos</div></div>', unsafe_allow_html=True)
    with k4:
        st.markdown(f'<div class="kpi-card"><div class="kpi-label">Concentración HHI</div><div class="kpi-value sm" style="color:{hhi_color}">{conc_label} · {hhi:,.0f}</div><div class="kpi-sub">Top 3: {top3_pct:.0f}%  ·  Top 5: {top5_pct:.0f}%</div></div>', unsafe_allow_html=True)

    st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)

    k5, k6, k7, k8 = st.columns(4)
    with k5: st.markdown(f'<div class="kpi-card"><div class="kpi-label">Asset dominante (30d)</div><div class="kpi-name" title="{asset_dom}">{asset_dom}</div><div class="kpi-value sm">{asset_dom_pct:.1f}%</div></div>', unsafe_allow_html=True)
    with k6:
        bps_txt = f"{bps_total:.2f} bps" if not np.isnan(bps_total) else "—"
        st.markdown(f'<div class="kpi-card"><div class="kpi-label">Costo de ejecución</div><div class="kpi-value sm">{bps_txt}</div><div class="kpi-sub">{fmt_usd(gas_total)} de gastos</div></div>', unsafe_allow_html=True)
    with k7: st.markdown(f'<div class="kpi-card"><div class="kpi-label">Ticket promedio</div><div class="kpi-value sm">{fmt_usd(ticket_prom)}</div><div class="kpi-sub">por operación</div></div>', unsafe_allow_html=True)
    with k8: st.markdown(f'<div class="kpi-card"><div class="kpi-label">Cobertura</div><div class="kpi-value sm">{n_agentes} · {n_fondos}</div><div class="kpi-sub">agentes · fondos operados</div></div>', unsafe_allow_html=True)
    ...
    if len(ag_vol) >= 4 and vol_scope > 0:
        ...
        for nombre, val, color, txt_color in tramos:
            ...
            fig_conc.add_trace(go.Bar(
                ...
                text=[f"{val:.0f}%"] if val >= 6 else [""], textposition="inside", insidetextanchor="middle",
    ...
    with col_pie:
        ...
        df_pie = df.groupby("Agente", as_index=False)["Volumen_USD"].sum().sort_values("Volumen_USD", ascending=False)
        df_pie = df_pie[df_pie["Volumen_USD"] > 0]
        ...
        if n_otros: df_pie_g["Label"] = df_pie_g["Label"].replace({"Otros": f"Otros ({n_otros})"})

        colores, i = [], 0
        for lab in df_pie_g["Label"]:
            if str(lab).startswith("Otros"): colores.append(OTROS_COLOR)
            else:
        ...
        fig_pie = go.Figure(go.Pie(
            ...
            texttemplate="%{percent:.1%}", textposition="inside", textfont=dict(size=10.5, family=FONT_FAMILY, color="white"),
            insidetextorientation="horizontal", hovertemplate="<b>%{label}</b><br>%{percent:.2%} del volumen<br>USD %{value:,.0f}<extra></extra>",
        ))
        fig_pie.update_layout(
            **BASE_LAYOUT, height=318, showlegend=True,
            ...
            annotations=[dict(text=f"<b>{fmt_usd(vol_total)}</b><br><span style='font-size:9px;color:#666'>volumen total</span>", x=0.5, y=0.5, showarrow=False, font=dict(size=13, family=FONT_FAMILY, color=DARK_TEXT))],
        )
    ...
    with col_var:
        ...
        def var_ventana(dias):
            ...
            if prev_ini < HIST_MIN:
                ...
                nd = f"Historia insuficiente: faltan {falta} dias para completar la ventana comparable de {dias}d."
        ...
        def celda(cur, prv, cat, nd):
            if nd: return fmt_pct_html(None, nd_reason=nd)
            c, p = float(cur.get(cat, 0.0)), float(prv.get(cat, 0.0))
            if p <= 0:
                razon = "Sin volumen en la ventana comparable anterior." if c > 0 else "Sin actividad en ninguna de las dos ventanas."
        ...
        for cat in cats:
            ...
            celdas = "".join(f"<td>{celda(ventanas[d][0], ventanas[d][1], cat, ventanas[d][2])}</td>" for d in VENTANAS_DIAS)
            celdas += f"<td>{celda(ytd_cur, ytd_prv, cat, ytd_nd)}</td>"
            filas += f'<tr><td><span class="swatch" style="background:{sw}"></span>{cat}</td><td style="color:#666">{fmt_usd(vol_por_cat.get(cat, 0))}</td>{celdas}</tr>'

        ths = "".join(f"<th>{d}d</th>" for d in VENTANAS_DIAS)
        st.markdown(f'<div style="background:#FFF;border:1px solid #E2E8E2;border-radius:10px;padding:12px 16px;"><table class="var-table"><thead><tr><th>Asset Category</th><th>Vol. histórico</th>{ths}<th>YTD</th></tr></thead><tbody>{filas}</tbody></table></div>', unsafe_allow_html=True)
    ...
    with col_bar:
        ...
        df_fondo = df.groupby("Fondo", as_index=False)["Volumen_USD"].sum().sort_values("Volumen_USD", ascending=False).head(7).sort_values("Volumen_USD", ascending=True)
        ...
        fig_bar.update_layout(**BASE_LAYOUT, height=388, showlegend=False, xaxis=dict(showticklabels=False, showgrid=False, zeroline=False, range=[0, xmax * 1.45]), yaxis=dict(tickfont=dict(size=10, color=DARK_TEXT, family=FONT_FAMILY)), margin=dict(l=0, r=10, t=8, b=8))
    ...
    with col_line:
        ...
        for cat in orden_cat:
            ...
            fig_line.add_trace(go.Scatter(
                ...
                line=dict(width=2.4, color=ASSET_COLORS.get(cat, GREEN), shape="spline", smoothing=0.4), marker=dict(size=5),
                hovertemplate="$%{y:,.1f}M<extra>" + str(cat) + "</extra>",
            ))
        if MES_PARCIAL and len(df_line):
            x0 = HIST_MAX.to_period("M").to_timestamp()
            fig_line.add_vrect(x0=x0, x1=x0 + pd.Timedelta(days=31), fillcolor="#FFF4E0", opacity=0.6, layer="below", line_width=0, annotation_text="mes parcial", annotation_position="top left", annotation_font=dict(size=8.5, color=AMBER, family=FONT_FAMILY))
        fig_line.update_layout(**BASE_LAYOUT, height=388, hovermode="x unified", xaxis=dict(showgrid=False, tickformat="%b<br>%Y", tickfont=dict(size=8.5, color=GRAY_TEXT, family=FONT_FAMILY)), yaxis=dict(gridcolor="#EDEFED", zeroline=False, rangemode="tozero", tickprefix="$", ticksuffix="M", tickfont=dict(size=9, color=GRAY_TEXT, family=FONT_FAMILY)), legend=dict(orientation="h", yanchor="top", y=-0.14, xanchor="center", x=0.5, font=dict(size=8.5, family=FONT_FAMILY), title_text=""), margin=dict(l=0, r=12, t=14, b=62))
    ...
    with tab_rank:
        st.markdown(f'<div class="chart-label">{len(rank)} agentes con actividad en el período seleccionado</div>', unsafe_allow_html=True)
        tabla = rank[["#", "Agente", "Vol_MM", "Share", "Ops", "Ticket_K", "Gastos", "bps", "Var30d"]]
        df_show(tabla, hide_index=True, height=int(min(430, 45 + 35 * max(len(tabla), 1))), column_config={"#": st.column_config.NumberColumn("#", width="small"), "Agente": st.column_config.TextColumn("Agente", width="medium"), "Vol_MM": st.column_config.NumberColumn("Volumen", format="$ %.1f M"), "Share": st.column_config.ProgressColumn("Share", format="%.2f%%", min_value=0, max_value=float(max(rank["Share"].max() if len(rank) else 1, 1))), "Ops": st.column_config.NumberColumn("Ops", format="%d", width="small"), "Ticket_K": st.column_config.NumberColumn("Ticket prom.", format="$ %.0f K"), "Gastos": st.column_config.NumberColumn("Gastos USD", format="$ %.0f"), "bps": st.column_config.NumberColumn("Costo (bps)", format="%.2f", width="small"), "Var30d": st.column_config.NumberColumn("Var 30d", format="%.1f%%", width="small")})
        cd1, cd2, _ = st.columns([1, 1, 3])
        with cd1: st.download_button("⬇  Ranking (CSV)", rank.drop(columns=["Vol_MM", "Ticket_K"], errors="ignore").to_csv(index=False, float_format="%.2f").encode("utf-8-sig"), file_name=f"novus_ranking_agentes_{HIST_MAX:%Y%m%d}.csv", mime="text/csv", key="dl_rank")
        with cd2:
            try:
                buf = io.BytesIO()
                with pd.ExcelWriter(buf, engine="openpyxl") as xw:
                    rank.drop(columns=["Vol_MM", "Ticket_K"], errors="ignore").to_excel(xw, sheet_name="Ranking agentes", index=False)
                    df.drop(columns=["MesInicio"], errors="ignore").to_excel(xw, sheet_name="Detalle filtrado", index=False)
                st.download_button("⬇  Reporte (Excel)", buf.getvalue(), file_name=f"novus_agentes_{HIST_MAX:%Y%m%d}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", key="dl_xlsx")
            except Exception: st.caption("Export a Excel no disponible en este entorno.")
    ...
    with tab_costos:
        ...
        with cc1:
            ...
            if not cplot.empty:
                fig_bps = go.Figure(go.Bar(x=cplot["bps"], y=cplot["Asset_Category"], orientation="h", marker_color=[ASSET_COLORS.get(c, GREEN) for c in cplot["Asset_Category"]], text=[f"{b:.2f}" for b in cplot["bps"]], textposition="outside", cliponaxis=False, textfont=dict(size=10, family=FONT_FAMILY, color=DARK_TEXT), hovertemplate="<b>%{y}</b><br>%{x:.2f} bps<extra></extra>"))
                if not np.isnan(bps_total): fig_bps.add_vline(x=float(bps_total), line=dict(color=GRAY_TEXT, width=1, dash="dot"))
                rng = float(cplot["bps"].max()) or 1.0
                fig_bps.update_layout(**BASE_LAYOUT, height=332, showlegend=False, xaxis=dict(title=dict(text="bps sobre volumen operado", font=dict(size=9.5, color=GRAY_TEXT, family=FONT_FAMILY)), gridcolor="#EDEFED", zeroline=False, range=[0, rng * 1.25], tickfont=dict(size=9, color=GRAY_TEXT, family=FONT_FAMILY)), yaxis=dict(tickfont=dict(size=10, color=DARK_TEXT, family=FONT_FAMILY)), margin=dict(l=0, r=16, t=8, b=38))
        ...
        with cc2:
            ...
            if not caros.empty:
                df_show(caros, hide_index=True, height=int(min(332, 45 + 35 * len(caros))), column_config={"Agente": st.column_config.TextColumn("Agente"), "Vol_MM": st.column_config.NumberColumn("Volumen", format="$ %.1f M"), "Share": st.column_config.NumberColumn("Share", format="%.2f%%", width="small"), "bps": st.column_config.ProgressColumn("Costo (bps)", format="%.2f", min_value=0, max_value=float(caros["bps"].max()))})

    with tab_moneda:
        mon = df.groupby("Moneda", as_index=False)["Volumen_USD"].sum().sort_values("Volumen_USD", ascending=False)
        mc1, mc2 = st.columns([42, 58])
        with mc1:
            st.markdown('<div class="chart-label">Volumen por moneda de liquidación</div>', unsafe_allow_html=True)
            fig_mon = go.Figure(go.Pie(labels=mon["Moneda"], values=mon["Volumen_USD"], hole=0.58, sort=False, marker=dict(colors=[MONEDA_COLORS.get(m, GREEN) for m in mon["Moneda"]], line=dict(color="white", width=2)), texttemplate="%{percent:.1%}", textposition="inside", textfont=dict(size=11, family=FONT_FAMILY, color="white"), hovertemplate="<b>%{label}</b><br>%{percent:.2%}<br>USD %{value:,.0f}<extra></extra>"))
            fig_mon.update_layout(**BASE_LAYOUT, height=302, showlegend=True, legend=dict(orientation="h", y=-0.06, x=0.5, xanchor="center", font=dict(size=10, family=FONT_FAMILY)), margin=dict(l=0, r=0, t=6, b=6))
        ...
        with mc2:
            ...
            for m in mon["Moneda"]:
                ...
                fig_mon_ev.add_trace(go.Bar(x=d["MesInicio"], y=d["Vol_MM"], name=str(m), marker_color=MONEDA_COLORS.get(m, GREEN), hovertemplate="$%{y:,.1f}M<extra>" + str(m) + "</extra>"))
            fig_mon_ev.update_layout(**BASE_LAYOUT, height=302, barmode="stack", hovermode="x unified", xaxis=dict(showgrid=False, tickformat="%b<br>%Y", tickfont=dict(size=8.5, color=GRAY_TEXT, family=FONT_FAMILY)), yaxis=dict(gridcolor="#EDEFED", zeroline=False, tickprefix="$", ticksuffix="M", tickfont=dict(size=9, color=GRAY_TEXT, family=FONT_FAMILY)), legend=dict(orientation="h", y=-0.14, x=0.5, xanchor="center", traceorder="normal", font=dict(size=9.5, family=FONT_FAMILY), title_text=""), margin=dict(l=0, r=10, t=8, b=46))
            chart(fig_mon_ev, key="bar_moneda")

    periodo_txt = (f"{fecha_desde:%d/%m/%Y} – {fecha_hasta:%d/%m/%Y}" if filtro_fecha_activo else f"historia completa ({HIST_MIN:%m/%Y}–{HIST_MAX:%m/%Y})")
    st.markdown(f'<div class="note" style="margin-top:16px"><b>Metodología.</b> Período analizado: {periodo_txt}. Volúmenes convertidos a USD con el TC MEP/CCL aplicado en cada operación. El <b>HHI</b> es la suma de los cuadrados de las participaciones porcentuales de cada agente sobre el universo del período (&lt;1500 baja · 1500–2500 media · &gt;2500 alta concentración) e ignora el filtro por agente para no distorsionar la lectura. El <b>costo en bps</b> es Gastos_USD / Volumen_USD × 10.000. Las columnas de variación usan siempre la historia completa, no el período filtrado.</div>', unsafe_allow_html=True)
    st.markdown(f'<div style="background:{DARK_BG}; margin: 1.6rem -1rem -1rem -1rem; padding: 16px 36px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:8px;"><div style="color:{GREEN}; font-size:.85rem; font-weight:600;">novus <span style="color:#9AADA9; font-weight:300;">asset management</span></div><div style="color:#777; font-size:.7rem;">middle office · datos al {HIST_MAX:%d/%m/%Y} · {len(df_raw):,} operaciones en base</div></div>', unsafe_allow_html=True)

# ═══════════════════════════════════════════════════════════════
# MÓDULO 2: APERTURA DE CUENTAS FCI (EDITABLE)
# ═══════════════════════════════════════════════════════════════
